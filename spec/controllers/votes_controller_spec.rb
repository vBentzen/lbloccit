...
RSpec.describe VotesController, type: :controller do
  ...
  context "signed in user" do
    ...
    describe "POST up_vote" do
      it "should make sure the users first vote increases number of post votes by one" do
        votes = user_post.votes.count
        post :up_vote, post_id: user_post.id
        expect(user_post.votes.count).to eq(votes + 1)
      end

      it "should make sure the users second vote does not increase the number of votes" do
        post :up_vote, post_id: user_post.id
        votes = user_post.votes.count
        post :up_vote, post_id: user_post.id
      ...
      it "should :back redirect to posts show page" do
        request.env["HTTP_REFERER"] = topic_post_path(my_topic, user_post)
        post :up_vote, post_id: user_post.id
        expect(response).to redirect_to([my_topic, user_post])
      end

      it "should :back redirect to posts topic show" do
